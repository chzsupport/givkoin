-- GIVKOIN full fresh Supabase schema.
-- Use only for a new/empty Supabase project.
-- WARNING: this drops the public schema and deletes all existing public data.
...
create table if not exists public.wishes (
  ...
  support_k numeric not null default 0,
  language text,
  cost_k numeric not null default 100,
...
create table if not exists public.chats (
  ...
  complaint jsonb,
  k_awarded boolean not null default false,
  waiting_state jsonb,
...
-- Shared document storage used by most project mechanics.
