...
async function awardChatRewardsForChat(chatId) {
  ...
  const chatBoostPercentA = Math.max(25, Number(userAData?.shopBoosts?.chatSc?.bonusPercent) || 25);
  const chatBoostPercentB = Math.max(25, Number(userBData?.shopBoosts?.chatSc?.bonusPercent) || 25);
