...
const getMe = async (req, res, next) => {
  try {
    ...
    // Личная удача (раз в сутки)
    const lastLucky = userObj.achievementStats?.lastLuckyDrawAt;
    const now = new Date();
    const alreadyToday = await hasClaimedPersonalLuckToday({
      userId: userObj._id,
      now,
      fallbackLastLuckyDrawAt: lastLucky,
    });
    userObj.luckyDayAvailable = !alreadyToday;
    userObj.newsCard = await getNewsUserCard({ userId: userObj._id, now }).catch(() => null);

    const supabase = getSupabaseClient();
    const { data: entityRow, error: entityError } = await supabase
      .from('entities')
      .select('id,user_id,name,stage,mood,avatar_url,satiety_until,created_at,updated_at,history')
      .eq('user_id', String(userObj._id))
      .maybeSingle();
    if (!entityError && entityRow) {
      const moodDiag = await getMoodDiagnosticsForUser(userObj._id).catch(() => null);
      userObj.entity = {
        _id: entityRow.id,
        user: entityRow.user_id,
        name: entityRow.name,
        stage: entityRow.stage,
        mood: moodDiag?.mood || entityRow.mood,
